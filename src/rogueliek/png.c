...
static int loadDataPng(const char *file, unsigned char **tex)
{
	FILE *fp = fopen(file, "rb");
	if(!fp){
		printf("Couldn't open file: %s\n", file);
...
{
	_png_texture_t ob = {0};
	if(!getSizePng(file, &ob.width, &ob.height)){
		return -1;
	}

	ob.pixels = (unsigned char*)malloc(ob.width * ob.height * 4);
	if(!loadDataPng(file, &ob.pixels)){
		free(ob.pixels);
		return -1;
	}

	ob.name = (char*)malloc(strlen(name) + 1);
	strcpy(ob.name, name);

	pngs = (_png_texture_t*)realloc(pngs, ++npngs * sizeof(_png_texture_t));
	pngs[npngs - 1] = ob;
